...
Xenon100LXeSensitiveDetector::Xenon100LXeSensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("LXeHitsCollection");
}

Xenon100LXeSensitiveDetector::~Xenon100LXeSensitiveDetector()
{
}

void Xenon100LXeSensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pLXeHitsCollection = new Xenon100LXeHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool Xenon100LXeSensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *pHistory)
{
	G4double dEnergyDeposited = pStep->GetTotalEnergyDeposit();
	G4Track *pTrack = pStep->GetTrack();

	Xenon100LXeHit* pHit = new Xenon100LXeHit();
...
void Xenon100LXeSensitiveDetector::EndOfEvent(G4HCofThisEvent *pHitsCollectionOfThisEvent)
{
//  if (verboseLevel>0) { 
